...
P_P = zeros(Ntot,1);                  % recording phase of the eye movement
P_G = zeros(Ntot,1);                  % recording gain of the eye movement
Cf_P = zeros(Ntot,1);                  % recording phase of the eye movement
Cf_G = zeros(Ntot,1);                  % recording gain of the eye movement
...
P_G(:)= P_G(:)/P_G(2*nit);
P_P = (P_P /T_pat)*360;
P_P = mod(360-P_P -269, 360);
P_P = mod(P_P +10, 360)-10;


% Renormalise phase and gain
Cf_G(:)= Cf_G(:)/Cf_G(2*nit);
Cf_P = (Cf_P /T_pat)*360;
Cf_P = mod(360-Cf_P -269, 360);
Cf_P = mod(Cf_P +10, 360)-10;